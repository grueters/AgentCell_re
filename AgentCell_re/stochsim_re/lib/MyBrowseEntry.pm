# MyBrowseEntry is a modified version of BrowseEntry, which return the index of choice
# in addition of its value  -- Nicolas Le Novère (on the suggestion of Slaven Rezic ) --
##
# BrowseEntry is a stripped down version of ComboBox.tcl from Tix4.0

package Tk::MyBrowseEntry;
...
Construct Tk::Widget 'MyBrowseEntry';
...
sub Populate {
    ...
    $c->withdraw;
    my $sl = $c->Scrolled( qw/Listbox -selectmode browse -scrollbars osoe/ );
...
sub LbChoose {
    ...
    if ((($x < 0) || ($x > $l->Width)) ||
       ...
       $w->LbCopySelection;
       $w->Callback(-browsecmd => $w, $w->Subwidget('entry')->get, $index);
    }
}
