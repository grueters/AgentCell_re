...
# File: writesbml.pm
#
# Aim:  Writes the simulation under the SBML format level 1  and level 2
...
#
#     Nicolas Le Novère 
#     Réceptors and Cognition, Pasteur Institute
...
sub writesbml1{
    ...
    $writer->startTag('model',
		     'name' => 'model'
		     ); # I cannot use $dirname in 'id' because there could be forbidden characters 
...
######## Put a description of the model here.
    ...
    $writer->startTag('compartment',
		     'name'              => 'comp1',
		     'volume'              => $sim->{'general_config'}{'ReactionVolume'},
		     );
...
# we should take care of the relevant states
...
###########################################################################
	...
	$writer->emptyTag('parameter',
			  'name'  => "kf",
			  'value' =>  $sim->{"reactions"}[$react_idx]{"Kf"}
			  );
	$writer->emptyTag('parameter',
			  'name'  => "kr",
			  'value' =>  $sim->{"reactions"}[$react_idx]{"Kr"}
			  );
...
###########################################################################
	...
	my $formula = "kf_$react_idx * ".
	    join(" * ", @{ $sim->{"reactions"}[$react_idx]{"Substrates"} }).
		" - kr_$react_idx * ".
		    join(" * ", @{ $sim->{"reactions"}[$react_idx]{"Products"} });
