...
package Tk::MyBalloon;

use vars qw($VERSION);
$VERSION = '3.026'; # $Id: MyBalloon.pm,v 1.1 2007/02/25 00:02:14 lenov Exp $

use Tk qw(Ev Exists);
use Carp;
require Tk::Toplevel;

Tk::Widget->Construct('MyBalloon');
...
sub ClassInit {
    my ($class, $mw) = @_;
    $mw->bind('all', '<Motion>', ['MyBalloon::Motion', Ev('X'), Ev('Y'), Ev('s')]);
    $mw->bind('all', '<Leave>',  ['MyBalloon::Motion', Ev('X'), Ev('Y'), Ev('s')]);
    $mw->bind('all', '<Button>', 'MyBalloon::ButtonDown');
    $mw->bind('all', '<ButtonRelease>', 'MyBalloon::ButtonUp');
