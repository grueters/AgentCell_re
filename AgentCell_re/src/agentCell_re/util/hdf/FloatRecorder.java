...
public class FloatRecorder {
    ...
    public FloatRecorder(int initialSize) {
    ...
    // Record an item.
    public void record(int index, float value) {
        // Check the index.
        if (index < 0) {
            // Note an error.
            System.out.println(
            ...
            if (index >= this.data.length) {
                ...
                float tempData[] = new float[Math.max(index + 1,
    ...
    public float[] getData() {
        ...
        if (this.recordedLength < this.data.length) {
            ...
            tempData = new float[this.recordedLength];
