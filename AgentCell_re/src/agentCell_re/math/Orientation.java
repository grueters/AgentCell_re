 ...
 *
 * The orientation O is a 3x3 rotation matrix that transforms the coordinates of a point from
 *  the body referential (attached to the body) into the laboratory referential (fixed). The column
 * vectors of O are the coordinates (in the laboratory referential) of the basis vectors of the body
 * referential.
 *        x  = coordinates of a point in the body referential
 *        x' = coordinates of a point in the laboratory referential
 *        O = orientation
 *        x' = O . x
 *
 * If we apply on the body a rotation R around a global (laboratory) axis, we obtain:
 *        x_new  = x remains unchanged: the point is fixed in the body
 *        x'_new = R . x' = R .O . x = new coordinates in laboratory referential
 *           O_new = R . O = new orientation of the body
 *
 * If we apply on the body a rotation R around one of its local axes we obtain:
 *                   x_new = x, remains unchanged: the point is fixed in the body
 *        x'_new = O . R . x = new coordinates in laboratory referential
 *            O_new = O . R = new orientation of the body
 */
public class Orientation extends RotationMatrix3x3 {
     ...
     * @param zz
     */
    public Orientation(double xx, double xy, double xz, double yx, double yy,
     ...
     * @param angle
     * @return this = R . this   (R the rotation matrix)
     * Rotate the orientation around one of the global (laboratory) axes
     */
    ...
    /**
     * @param axisVect
     *  coordinates of the axial vector in the global  (laboratory) referential
     * (does not need to have length one)
     * @param angle
     * @return this = R . this   (R the rotation matrix)
     * Rotate the orientation around the axial vector with global (laboratory) coordinates axialVect
     */
     ...
     */
    public Orientation rotateAroundGlobalAxes(int axesOrder, double angle0,
     ...
     * Rotate orientation around 3 global axes in a given order.
     */
    public Orientation rotateAroundGlobalAxes(int axesOrder, double c0,
        double c1, double c2, double s0, double s1, double s2) {
     ...
     * @param axis : index of the axis of rotation
     * @param angle
     * @return this = this . R  (R the rotation matrix)
     * Rotate the orientation around one of its local (body) axes
     */
    ...
    /**
     * @param axisVect
     * coordinates of the axial vector in the local (body) referential
     * (does not need to have length one)
     * @param angle
     * @return this = this . R   (R the rotation matrix)
     * Rotate the orientation around the axial vector with local (body) coordinates axialVect
     */
     ...
     */
    public Orientation rotateAroundLocalAxes(int axesOrder, double angle0,
        double angle1, double angle2) {
        rotationMatrix.setAxesAngles(axesOrder, angle0, angle1, angle2);
        this.mult(rotationMatrix);
        applyGramSchmidtIfNecessary();
     ...
     * Rotate orientation around 3 local axes in a given order.
     */
    public Orientation rotateAroundLocalAxes(int axesOrder, double c0,
        double c1, double c2, double s0, double s1, double s2) {
        rotationMatrix.setAxesAngles(axesOrder, c0, c1, c2, s0, s1, s2);
        this.mult(rotationMatrix);
        applyGramSchmidtIfNecessary();
    ...
    /**
     * Randomizing the orientation:
     * 1) we reset the orientation to identity
     * 2) we choose phi = 2 Pi u, theta = acos(2v-1) with u and v
     *    uniformly distributed between 0 and 1. See e.g.
     *    http://mathworld.wolfram.com/SpherePointPicking.html
     *    (note that unlike us they call the longitude theta and the latitude phi)
     * 3) we chose a new direction of the local z axis by:
     *       rotating around the local z axis by phi
     *       rotating around the local y axis by theta
     * 4) keeping the local z axis fixed we randomize the orientation of the 2 other local axes
     *    by rotating around the local z axis for an angle alpha uniformly distributed between
     *    0 and 2Pi.
     * @return
     */
    public Orientation randomize() {
        ...
        double theta = Math.acos((2 * RandomHelper.nextDoubleFromTo(0, 1)) -
                1);
    ...
    /**
     * @return reference to the the 3rd column vector of the Orientation matrix
     * Note that further changes in the returned vector will be reflected in the orientation matrix
     */
    ...
    public void setRotationsCountThreshold(int i) {
        System.out.println(
            "WARNING: you are changing the rotationsCountThreshold " +
            "from the default value, " + this.getRotationsCountThreshold() +
            ",  to the new value " + i +
            ". You should check that your orientation " +
            "matrix is still orthogonal.");
        rotationsCountThreshold = i;
    }
    
    /**
    This requires a pure rotation matrix 'm' as input.
    taken from: https://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToAngle/index.htm
    */
    public float[] toAxisAngle() {
      double angle,x,y,z; // variables for result
    	...
    	if ((Math.abs(rotationMatrix.getElement(0, 1)-rotationMatrix.getElement(1, 0))< epsilon)
    	  ...
    	  && (Math.abs(rotationMatrix.getElement(1, 2)-rotationMatrix.getElement(2, 1))< epsilon)) {
    		...
    		if ((Math.abs(rotationMatrix.getElement(0, 1)+rotationMatrix.getElement(1, 0)) < epsilon2)
    		  ...
    		  && (Math.abs(rotationMatrix.getElement(0, 0)+rotationMatrix.getElement(1, 1)+rotationMatrix.getElement(2, 2)-3) < epsilon2)) {
    	...
    	double s = Math.sqrt((rotationMatrix.getElement(2, 1) - rotationMatrix.getElement(1, 2))*(rotationMatrix.getElement(2, 1) - rotationMatrix.getElement(1, 2))
    		+(rotationMatrix.getElement(0, 2) - rotationMatrix.getElement(2, 0))*(rotationMatrix.getElement(0, 2) - rotationMatrix.getElement(2, 0))
    		+(rotationMatrix.getElement(1, 0) - rotationMatrix.getElement(0, 1))*(rotationMatrix.getElement(1, 0) - rotationMatrix.getElement(0, 1))); // used to normalise
    	if (Math.abs(s) < 0.001) s=1; 
    		// prevent divide by zero, should not happen if matrix is orthogonal and should be
    		// caught by singularity test above, but I've left it in just in case
    	angle = Math.acos(( rotationMatrix.getElement(0, 0) + rotationMatrix.getElement(1, 1) + rotationMatrix.getElement(2, 2) - 1)/2);
